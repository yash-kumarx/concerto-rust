//! Loads model files and looks types up across namespaces.
//!
//! The [`ModelManager`] is the one part of the core that holds state. It owns
//! the loaded [`ModelFile`]s and handles anything that needs to see more than
//! one namespace at once: resolving a type (local, imported, or through a
//! wildcard), collecting every property up an inheritance chain, and checking
//! whether one type can stand in for another. With all that living here, the
//! validator, once it's written, can stay a plain function over models that
...
impl ModelManager {
    ...
    /// Looks up a declaration by its full name.
    ///
    /// The version doesn't matter for matching, so `org.acme@1.0.0.Foo` and
    /// `org.acme.Foo` land on the same declaration.
    ...
    /// Takes a short name as written inside `in_namespace` and gives back its
    /// full name. Checks primitives, local declarations and named imports
    /// first, and only then falls back to wildcard (`ns.*`) imports.
    ...
    /// all of its super types. Hard-errors if the name isn't a concept-like
    /// type, a super type doesn't resolve, or the chain loops back on itself.
    ...
    /// True if a `sub_fqn` value also counts as a `super_fqn` value: either
    /// they're the same type, or `sub_fqn` extends `super_fqn` somewhere up
    /// the chain.
