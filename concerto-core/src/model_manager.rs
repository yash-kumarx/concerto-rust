...
//! loaded [`ModelFile`]s and provides the cross-namespace operations the
//! validator needs — type resolution (local, imported, and wildcard-imported),
//! inheritance-aware property collection, and assignability checks. Keeping
//! this book-keeping here lets the (future) validator stay a pure function of
//! already-resolved model state.

use std::collections::{HashMap, HashSet};

use crate::error::{ConcertoError, Result};
use crate::model::declaration::{ClassDeclaration, Declaration};
use crate::model::model_file::ModelFile;
...
impl ModelManager {
    /// Creates a manager preloaded with the `concerto@1.0.0` system model.
    ...
    /// Adds a model from its JSON AST. Errors on a duplicate namespace.
    pub fn add_model(
        &mut self,
    ...
    /// Returns the model file for a namespace, if loaded.
    pub fn model_file(&self, namespace: &str) -> Option<&ModelFile> {
        self.model_files.get(namespace)
    }

    /// Resolves a fully-qualified name to its declaration.
    ///
    /// Matching is version-insensitive: `org.acme@1.0.0.Foo` and
    ...
    pub fn get_declaration(&self, fqn: &str) -> Result<&Declaration> {
        ...
        if let Some(mf) = self.model_files.get(ns) {
            if let Some(decl) = mf.local_declaration(short) {
                return Ok(decl);
            }
        }
    ...
    /// Resolves a short type name, as written inside `in_namespace`, to a
    /// fully-qualified name. Tries primitives, local declarations and explicit
    /// imports first, then wildcard (`ns.*`) imports.
    pub fn resolve_type_name(&self, in_namespace: &str, short: &str) -> Result<String> {
        ...
        for imp in mf.imports().iter().filter(|i| i.is_wildcard()) {
            if let Some(imported) = self.model_files.get(imp.namespace()) {
                if imported.local_declaration(short).is_some() {
    ...
    /// Collects every property of a type, walking the full inheritance chain
    /// from the type up through its super types. Returns a hard error if the
    /// type is not a concept-like declaration, a super type cannot be
    /// resolved, or the inheritance chain is circular.
    ...
    /// Returns `true` if an instance of `sub_fqn` is a valid instance of
    /// `super_fqn` — i.e. they are the same type or `sub_fqn` transitively
    /// extends `super_fqn`.
    ...
    /// Walks the inheritance chain of a class type, returning each
    /// `(fully-qualified-name, declaration)` from the type up to its root.
    ...
    /// Resolves the fully-qualified name of a class's direct super type, in the
    /// context of the namespace that declares it.
    fn super_type_fqn(
        &self,
...
/// Strips the `@version` from a namespace string.
...
/// Normalizes a fully-qualified name by removing its namespace version, so
/// FQNs can be compared regardless of whether they carry a version.
