//! Typed representation of a declaration's properties.
//!
//! The generated metamodel deserializes every property into the base
//! [`mm::Property`] struct, which drops subtype-specific fields (validators,
//! the referenced `type` of object/relationship properties, and so on). To
//! preserve that information we re-read each property from its raw JSON into
//! [`Property`], a sum type whose variant is chosen by the `$class`
//! discriminator. Accessors are inherent methods on the enum rather than a
//! trait hierarchy (see `AGENTS.md`: prefer sum types over complex traits).
...
impl Property {
    ...
    /// The name of the property's type: the primitive name for primitives, or
    /// the referenced type name for object/relationship properties. `None`
    /// for enum value members, which have no type of their own.
...
impl TryFrom<&serde_json::Value> for Property {
    ...
    fn try_from(value: &serde_json::Value) -> Result<Self> {
        ...
        // Deserialize into the matching metamodel struct. A failure here means
        // the AST is malformed for the kind its `$class` claims to be.
